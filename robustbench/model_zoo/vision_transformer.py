...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        # return x
        if self.dist_token is None: #yes
            return self.pre_logits(x[:, 0])       #线性映射
        else: 
            return x[:, 0], x[:, 1]

    def forward(self, x):
        x = self.forward_features(x)
        if self.head_dist is not None: #ignore
    ...
    #     # return x
    #     if self.dist_token is None: #yes
    #         return self.pre_logits(x[:, 0]),x_final       #线性映射
    #     else:
            
    #         return x[:, 0], x[:, 1]

    # def forward(self, x):
    #     x,x_final = self.forward_features(x)
    #     if self.head_dist is not None: #ignore
    #         x, x_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
    #         if self.training and not torch.jit.is_scripting():
    #             # during inference, return the average of both classifier predictions
